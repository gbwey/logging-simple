...
-- need this for windows -- hlint . --cpp-define=mingw32_HOST_OS
...
fastest possible time commands for windows and unix environment
-}
module DateTA where
...
#endif
import Data.Fixed (Pico, div', divMod')
import Data.Time
...
#ifdef mingw32_HOST_OS
-- | format 'SYSTEMTIME' in long format
...
-- | format 'SYSTEMTIME' in short format
...
-- | format zonedtime as text in long format
formatAsTimeStampP :: ZonedTime -> Text
formatAsTimeStampP zt = toText $
  TA.run2 $ do
